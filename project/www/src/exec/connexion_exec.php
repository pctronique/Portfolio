    ...
    if(!empty($sgbd)) {
        ...
        try {
            ...
            $res = $sgbd->prepare("SELECT * FROM utilisateur LEFT JOIN admin ON utilisateur.id_admin = admin.id_admin".
                            " WHERE login_user=:login OR email_user=:login");
            ...
            if($res->rowCount() > 0) {
                ...
                foreach ($data as $valueLine) {
                    ...
                    if($valueLine['id_admin'] != 4) {
                        ...
                        if(Pass_Crypt::verify($_POST['password_user'], $valueLine['mot_pass_user'])) {
                            /* connecter l'utilisateur */
                            $_SESSION['id_user'] = $valueLine['id_user'];
                            $_SESSION['id_admin'] = $valueLine['id_admin'];
                            $_SESSION['nom'] = $valueLine['nom_user'];
                            $_SESSION['prenom'] = $valueLine['prenom_user'];
                            $_SESSION['login'] = $valueLine['login_user'];
                            $_SESSION['email'] = $valueLine['email_user'];
                            $_SESSION['avatar'] = $valueLine['avatar_user'];
                            echo "true";
                        } else {
