...
function load_msg(e) {
    ...
    fetch_post("./src/exec/msg_load_exec.php", values).then(function (response) {
        ...
        if (response.split("[#json#]")[0] == "true") {
            ...
            document.getElementById('msg_from').innerHTML = tabUser['Nom_msg']+ " "+ tabUser['Prenom_msg']+ " ("+ tabUser['Email_msg']+ ").";
            document.getElementById('msg_date').innerHTML = display_date(tabUser['date_msg'])+".";
            document.getElementById('msg_obj').innerHTML = tabUser['Objet_msg']+".";
            document.getElementById('msg_txt').innerHTML = tabUser['Message_msg'].replaceAll("\n", '<br />');
